...
import { resolveSalesLineUomFromRaw } from './uom';

type Db = { query: (text: string, params?: unknown[]) => Promise<{ rows: Record<string, unknown>[] }> };

/** Resolve line UOM for print/PDF — uses saved uom_id + conversion, not product default pc. */
export async function enrichSalesPrintLineUoms(db: Db, items: any[]): Promise<any[]> {
  ...
  if (productIds.length) {
    const prodRows = await db.query(
      'SELECT id, default_sales_uom_id FROM products WHERE id = ANY($1::uuid[])',
      [productIds],
    );
    for (const row of prodRows.rows) {
      defaultUomByProduct[String(row.id)] = row.default_sales_uom_id != null
        ? Number(row.default_sales_uom_id)
        : null;
    }
  }
  return items.map((row) => {
    const resolved = resolveSalesLineUomFromRaw(uomMap[row.product_id] || [], {
      uom_id: row.uom_id,
      conversion_to_base: row.conversion_to_base,
      entered_qty: row.entered_qty,
      quantity: row.quantity ?? row.ordered_qty,

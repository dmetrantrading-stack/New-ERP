...
type Props = {
  businessName?: string;
  canEdit?: boolean;
  onAccountClick?: (account: AccountClickPayload) => void;
  onReportSummaryChange?: (summary: ComparativeIncomeStatementData | null) => void;
};
...
export default function IncomeStatementTab({
  ...
  useEffect(() => {
    onReportSummaryChange?.(reportData);
  }, [reportData, onReportSummaryChange]);
  ...
  return (
    <div className="space-y-4 max-w-full">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-sm font-semibold text-slate-800">Profit and Loss Statement</h3>
        {canEdit && (
          <button
            type="button"
            onClick={openNew}
      ...
      <p className="text-xs text-gray-400">
        Saved reports store your comparative column setup. Use View to run the report, or Edit to change periods and options.
      </p>
    </div>
  );
}

...
export default function SalesInvoices() {
  ...
  const selectCustomer = (customerId: string) => {
    ...
    const mappedMode = ['Retail', 'Wholesale', 'Distributor'].includes(c.customer_type) ? c.customer_type : 'Wholesale';
  ...
  const viewInvoice = async (id: string) => {
    try { const res = await api.get(`/sales/invoices/${id}`); setShowInvoice(res.data); } catch { toast.error('Failed to load invoice'); }
  };
  ...
  if (creating) {
    return (
      <div className="h-[calc(100vh-4rem)] flex flex-col -m-6 bg-gray-50">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200 text-[10px] font-semibold text-gray-500 uppercase">
                    ...
                    <th className="px-2 py-2 text-left" style={{ minWidth: 600 }}>Product</th>
                        ...
                        <td className="px-2 py-2.5">
                          <ProductAutocomplete
                            ...
                            onSelect={(p) => {
                              ...
                              setForm({ ...form, items });
                              if (p.id) { api.get(`/inventory/product/${p.id}`).then(res => { const locInv = res.data.find((inv: any) => inv.location_id === 1); items[i].available_qty = locInv ? parseFloat(locInv.quantity) : 0; setForm(prev => ({ ...prev, items: [...items] })); }).catch(() => {}); }
                            }}
                          />
                        </td>
        ...
        {/* Attachments */}
        <AttachmentPanel referenceType="SalesInvoice" referenceId={currentInvoiceId || ''} />
  ...
  return (
    <div className="space-y-4">
            ...
            {invoices.map((inv) => {
              ...
              <tr key={inv.id}>
                <td className="font-mono text-xs">
                  {inv.invoice_number}
                  <button onClick={() => viewAttachments(inv.id)}
                    className="ml-1.5 inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded text-[9px] font-medium hover:bg-blue-100"
                    title="View Attachments">📎</button>
                </td>
                <td>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowInvoice(null)}>
          <div className="bg-white rounded-xl shadow-2xl flex flex-col w-[95vw] max-w-none max-h-[95vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-shrink-0 p-4 border-t border-gray-200 bg-gray-50">
              <AttachmentPanel referenceType="SalesInvoice" referenceId={showInvoice.id} />
            </div>
          </div>
        </div>
      )}
      ...
      {showAttachModal && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4" onClick={() => { setShowAttachModal(false); setPreviewAttachFile(null); }}>
          <div className="bg-white rounded-xl shadow-2xl flex flex-col w-[600px] max-h-[80vh]" onClick={(e) => e.stopPropagation()}>

...
const generateRefNumber = async (): Promise<string> => {
  const result = await query("SELECT COALESCE(MAX(CAST(SUBSTRING(expense_number, 3) AS INTEGER)), 0) + 1 as next FROM expenses WHERE expense_number ~ '^EX-'");
...
router.post('/', authenticate, auditLog('Expenses', 'Create'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const cat = await query('SELECT name FROM expense_categories WHERE id = $1', [category_id]);
    const accountCode = accountMap[cat.rows[0]?.name] || '6080';

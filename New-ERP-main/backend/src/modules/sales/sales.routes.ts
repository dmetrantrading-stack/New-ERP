...
router.get('/invoices/:id/print', async (req: any, res: Response) => {
  try {
    ...
    let decoded: any;
    try { decoded = jwt.verify(token, process.env.JWT_SECRET || 'd-metran-erp-secret-key-2024'); }
...
body{font-family:Arial,sans-serif;font-size:12px;color:#000;padding:15mm 12mm}
.header{display:flex;justify-content:space-between;border-bottom:2px solid #000;padding-bottom:10px;margin-bottom:8px}
.header h1{font-size:20px;font-weight:bold;margin-bottom:2px}
.header .sub{font-size:9px;color:#444}
.title{text-align:center;font-size:16px;font-weight:bold;letter-spacing:4px;margin:10px 0;border:1px solid #000;padding:4px}
.info{display:flex;justify-content:space-between;margin-bottom:10px}
.info-box{width:48%}
.info-box table{width:100%;border-collapse:collapse}
.info-box td{padding:1px 4px;font-size:11px}
.info-box td:first-child{font-weight:bold;width:35%}
.items-table{width:100%;border-collapse:collapse;margin-bottom:10px}
.items-table th{background:#eee;border:1px solid #000;padding:3px 4px;font-size:10px;text-align:center}
.items-table td{border:1px solid #000;padding:2px 4px;font-size:10px}
.items-table td.n{text-align:center}.items-table td.r{text-align:right}
.totals{display:flex;justify-content:flex-end;margin-bottom:16px}
.totals table{width:280px;border-collapse:collapse}
.totals td{padding:2px 6px;font-size:11px}
.totals td:first-child{font-weight:bold}.totals td:last-child{text-align:right;width:100px}
.totals .due{border-top:1px solid #000;font-size:13px;font-weight:bold}
.footer{display:flex;justify-content:space-between;margin-top:30px;border-top:1px solid #000;padding-top:10px}
.sig{text-align:center;width:22%}.sig .line{border-bottom:1px solid #000;margin-bottom:3px;height:40px}
.sig .label{font-size:9px}
.notes{margin-top:8px;font-size:9px;color:#555}
@media print{body{padding:10mm 10mm}}
</style></head><body>
<div class="header">
<div><h1>D METRAN TRADING</h1><div class="sub">Philippines</div><div class="sub">TIN: 000-000-000-000</div></div>
<div style="text-align:right"><div class="sub">Status: ${i.status}</div><div class="sub">Salesperson: ${i.cashier_name || ''}</div></div>
</div>
<div class="title">SALES INVOICE</div>
<div class="info">
<div class="info-box"><table>
<tr><td>Invoice #:</td><td>${i.invoice_number}</td></tr>
<tr><td>Date:</td><td>${new Date(i.invoice_date).toLocaleDateString('en-PH',{year:'numeric',month:'long',day:'numeric'})}</td></tr>
<tr><td>Due Date:</td><td>${i.due_date ? new Date(i.due_date).toLocaleDateString('en-PH',{year:'numeric',month:'long',day:'numeric'}) : '—'}</td></tr>
<tr><td>Payment:</td><td>${i.payment_method}</td></tr>
<tr><td>Terms:</td><td>${i.payment_terms || '—'}</td></tr>
</table></div>
<div class="info-box"><table>
<tr><td>Customer:</td><td>${i.customer_name || 'Walk-in'}</td></tr>
<tr><td>Code:</td><td>${i.customer_code || '—'}</td></tr>
<tr><td>TIN:</td><td>${i.customer_tin || '—'}</td></tr>
<tr><td>Address:</td><td>${i.customer_address || '—'}</td></tr>
<tr><td>Contact:</td><td>${i.customer_phone || '—'}</td></tr>
</table></div>
</div>

<table class="items-table">
<thead><tr>
<th>#</th><th>Description</th><th>Qty</th><th>Unit</th><th>Price</th><th>Disc</th><th>Total</th>
</tr></thead><tbody>
${items.rows.map((row: any, idx: number) => `<tr>
<td class="n">${idx+1}</td>
<td>${row.product_name||row.description||''}</td>
<td class="n">${parseFloat(row.quantity)}</td>
<td class="n">${row.unit_of_measure||''}</td>
<td class="r">${parseFloat(row.unit_price).toFixed(2)}</td>
<td class="n">${parseFloat(row.discount||0).toFixed(1)}%</td>
<td class="r">${parseFloat(row.total).toFixed(2)}</td>
</tr>`).join('')}
</tbody></table>

<div class="totals"><table>
<tr><td>Subtotal:</td><td>₱${parseFloat(i.subtotal).toFixed(2)}</td></tr>
<tr><td>Discount:</td><td>₱${parseFloat(i.discount).toFixed(2)}</td></tr>
<tr><td>VATable Sales:</td><td>₱${parseFloat(i.vatable_sales||0).toFixed(2)}</td></tr>
${parseFloat(i.vat_exempt_sales||0) > 0 ? `<tr><td>VAT Exempt:</td><td>₱${parseFloat(i.vat_exempt_sales).toFixed(2)}</td></tr>` : ''}
${parseFloat(i.zero_rated_sales||0) > 0 ? `<tr><td>Zero Rated:</td><td>₱${parseFloat(i.zero_rated_sales).toFixed(2)}</td></tr>` : ''}
<tr><td>VAT (12%):</td><td>₱${parseFloat(i.vat_amount||i.tax||0).toFixed(2)}</td></tr>
${parseFloat(i.lgu_final_tax||0) > 0 ? `<tr><td>LGU Final VAT 5%:</td><td>₱${parseFloat(i.lgu_final_tax).toFixed(2)}</td></tr>` : ''}
${parseFloat(i.withholding_tax||0) > 0 ? `<tr><td>Withholding Tax 1%:</td><td>₱${parseFloat(i.withholding_tax).toFixed(2)}</td></tr>` : ''}
<tr class="due"><td>Amount Due:</td><td>₱${parseFloat(i.total).toFixed(2)}</td></tr>
${i.payment_method==='Cash' ? `<tr><td>Amount Tendered:</td><td>₱${parseFloat(i.amount_paid).toFixed(2)}</td></tr><tr><td>Change:</td><td>₱${Math.max(0,parseFloat(i.amount_paid)-parseFloat(i.total)).toFixed(2)}</td></tr>` : ''}
</table></div>

<div style="font-size:10px;margin-bottom:4px"><strong>Total Items:</strong> ${items.rows.length} &ensp; <strong>Total Qty:</strong> ${totalQty} &ensp; <strong>Status:</strong> ${i.status}</div>
<div class="notes">${i.notes ? 'Notes: '+i.notes : ''}</div>
...
<p style="font-size:8px;text-align:center;margin-top:8px;color:#999">Printed: ${new Date().toLocaleString()} &mdash; This is a computer-generated document.</p>
</body></html>`;
...
router.patch('/invoices/:id/void', authenticate, auditLog('Sales', 'Void Invoice'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Reverse customer/employee balance
    if (inv.customer_type === 'Employee' && inv.employee_id) {
      await client.query('UPDATE employees SET grocery_credit_balance = grocery_credit_balance - $1 WHERE id = $2', [parseFloat(inv.total), inv.employee_id]);
    } else if (inv.customer_id) {
      const balance = parseFloat(inv.balance);
      await client.query('UPDATE customers SET balance = balance - $1 WHERE id = $2', [balance, inv.customer_id]);
    }
...
router.post('/collections', authenticate, auditLog('Sales', 'Create Collection'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Insert collection receipt (amount stored as cash_collected for historical accuracy)
    await query(
      ...
      [id, receipt_number, customer_id, invoice_id, collection_date || new Date(), payment_method, reference_number, bank_account_id || null, cashCollected, notes, req.user!.id]
    );
    ...
    // Update customer balance (reduce by applied amount)
    if (customer_id) {
    ...
    // Accounting entries — JE total = applied amount
    const entryId = uuidv4();
    const entryNumber = await generateRefNumber('JE', 'journal_entries', 'entry_number');
    ...
    // 2. EWT (1105) was already debited during invoice creation — included in invoice's AR net amount
    ...
    // 4. Credit AR (1100) for the cash collected amount (EWT already in 1105 from invoice)
    await query(
      `INSERT INTO journal_entry_lines (id, entry_id, account_id, description, debit, credit, reference_type, reference_id)
       VALUES ($1, $2, (SELECT id FROM chart_of_accounts WHERE account_code = '1100'), $3, 0, $4, 'Collection', $5)`,

...
const seed = async () => {
  ...
  try {
    ...
    const expenseCategories = [
      'Fuel', 'Transportation', 'Food Allowance', 'Pantry Supplies', 'Salaries',
      'Utilities', 'Repairs', 'Office Supplies', 'Miscellaneous'
    ];
    for (const cat of expenseCategories) {
      const exists = await client.query('SELECT id FROM expense_categories WHERE name = $1', [cat]);
      if (exists.rows.length === 0) {
        await client.query('INSERT INTO expense_categories (name) VALUES ($1)', [cat]);
      }
    }

...
import { BarChart3, TrendingUp, AlertTriangle, Clock, Download } from 'lucide-react';
            ...
            {loading ? <div className="text-center py-8 text-gray-400">Loading...</div> :
              activeReport === 'daily-sales' && (
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {data?.map((item: any) => <tr key={item.sku}><td className="font-mono text-xs">{item.sku}</td><td>{item.name}</td><td>{item.total_qty}</td><td>{formatCurrency(item.total_amount)}</td><td>{formatCurrency(item.total_cost)}</td><td className="font-bold">{formatCurrency(item.gross_profit)}</td><td>{item.margin_pct}%</td></tr>)}
                  </tbody>
                </table>
              ) || activeReport === 'sales-by-cashier' && (
                <table className="data-table">
                  <thead><tr><th>Cashier</th><th>Transactions</th><th>Total Sales</th><th>Avg Sale</th></tr></thead>
                  <tbody>
                    {data?.map((c: any) => <tr key={c.full_name}><td>{c.full_name}</td><td>{c.transaction_count}</td><td>{formatCurrency(c.total_sales)}</td><td>{formatCurrency(c.avg_sale)}</td></tr>)}
                  </tbody>
                </table>
              ) || activeReport === 'sales-by-customer' && (
                <table className="data-table">
                  <thead><tr><th>Code</th><th>Customer</th><th>Invoices</th><th>Total Sales</th><th>Balance</th></tr></thead>
                  <tbody>
                    {data?.map((c: any) => <tr key={c.customer_code}><td className="font-mono text-xs">{c.customer_code}</td><td>{c.customer_name}</td><td>{c.invoice_count}</td><td>{formatCurrency(c.total_sales)}</td><td className={c.total_balance > 0 ? 'text-red-600 font-medium' : ''}>{formatCurrency(c.total_balance)}</td></tr>)}
                  </tbody>
                </table>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {data?.map((i: any) => <tr key={`${i.sku}-${i.location_name}`}><td className="font-mono text-xs">{i.sku}</td><td>{i.name}</td><td>{i.location_name}</td><td>{i.total_quantity}</td><td>{formatCurrency(i.cost)}</td><td className="font-medium">{formatCurrency(i.total_value)}</td></tr>)}
                  </tbody>
                </table>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {data?.map((i: any) => <tr key={`${i.sku}-${i.location_name}`}><td className="font-mono text-xs">{i.sku}</td><td className="font-medium text-red-600">{i.name}</td><td>{i.location_name}</td><td className="text-red-600 font-bold">{i.quantity}</td><td>{i.reorder_level}</td><td className="text-red-600">{i.deficit}</td></tr>)}
                  </tbody>
                </table>
              ) || activeReport === 'expiry' && (
                <table className="data-table">

...
import { Plus, Eye, Send, ArrowLeft, Edit2 } from 'lucide-react';
...
export default function PurchaseOrders() {
  ...
  const loadOrders = () => {
    const url = supplierFilter ? `/purchases/orders?supplier_id=${supplierFilter}` : '/purchases/orders';
    api.get(url).then((res) => setOrders(res.data)).catch((err) => toast.error(err.response?.data?.error || 'Failed to load')).finally(() => setLoading(false));
  };
  ...
  const handleProductSearch = (value: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      setSearchResults(products.filter((p: any) => p.is_active !== false && (p.name?.toLowerCase().includes(q) || p.sku?.toLowerCase().includes(q) || p.barcode?.toLowerCase().includes(q))).slice(0, 15));
  ...
  const addItemFromSearch = (product: any) => {
    const newItem = {
      ...
      variant_id: '', location_id: 1, quantity: 1, unit_cost: product.cost || 0,
      unit_of_measure: product.unit_of_measure || '', variants: product.variants || [],
      available_qty: 0, discount_type: '%', discount_value: '0',
    };
  ...
  const resetForm = () => {
    ...
    setSelectedSupplier(null); setProductSearch(''); setEditingPO(null);
  };
  ...
  const openReceive = async (po: any) => {
    try {
      ...
      setReceiveForm({ location_id: 1, items: items.filter((i: any) => i.quantity > 0) });
      setShowReceive(true);
      ...
      <div className="h-[calc(100vh-4rem)] flex flex-col -m-6 bg-gray-50">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="relative mb-3">
            <input ref={searchInputRef} type="text" value={productSearch} onChange={(e) => handleProductSearch(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Escape') { setProductSearch(''); setShowSearchResults(false); } }}
              placeholder="Search product by name, SKU, or barcode (F2)..." className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10 max-h-64 overflow-y-auto">
                {searchResults.map((p: any) => (
                  <button key={p.id} onClick={() => addItemFromSearch(p)}
                    ...
                    <span className="text-gray-400 ml-2 text-xs">{p.sku} · {formatCurrency(p.cost || 0)}</span>
                  </button>
                ))}
              </div>
            )}
            {showSearchResults && productSearch && searchResults.length === 0 && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10 p-4 text-sm text-gray-400 text-center">No products found</div>
            )}
          </div>

...
export default function BankCashPage() {
  ...
  // Forms
  const [accountForm, setAccountForm] = useState<any>({ account_code: '', bank_name: '', account_name: '', account_number: '', account_type: 'Savings' });
  ...
  useEffect(() => {
    ...
    if (activeTab === 'transactions') {
      api.get('/bank-cash/cash-transactions').then(r => setCashTxns(r.data.data || r.data || [])).catch(() => {});
  ...
  const totalCash = accounts.reduce((s, a) => s + (a.computed_balance || a.balance || 0), 0);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {activeTab === 'accounts' && <button onClick={() => setShowAccountModal(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700"><Plus size={16} /> Add Account</button>}
          {activeTab === 'transactions' && (
            <>
              ...
              <button onClick={() => setShowBankTxnModal(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700"><Plus size={16} /> Bank Txn</button>
              ...
              <tbody>
                {accounts.map(a => (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="text-right font-bold">{formatCurrency(a.computed_balance || a.balance || 0)}</td>
              ...
              <div className="space-y-3">
                ...
                <div><label className="block text-sm font-medium mb-1">Type</label>
                  <select value={accountForm.account_type} onChange={e => setAccountForm({...accountForm, account_type: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    <option value="Savings">Savings</option><option value="Checking">Checking</option><option value="E-Wallet">E-Wallet</option><option value="Clearing">Clearing</option>
                  </select></div>
              </div>

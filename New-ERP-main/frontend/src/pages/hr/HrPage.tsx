...
import {
  ...
  CheckCircle, XCircle, Shield, UserCheck, CreditCard,
} from 'lucide-react';
...
export default function HrPage() {
  ...
  const [sssForm, setSssForm] = useState<any>({ employee_id: '', period_start: '', period_end: '', employer_amount: 0, notes: '' });
  const [payAccount, setPayAccount] = useState({ payment_account_type: 'cash', payment_account_id: '', payment_date: '' });
  ...
  useEffect(() => {
    const tab = activeTab;
    if (tab === 'payroll' || tab === 'payslip') { api.get('/hr/payroll').then((r) => setPayroll(r.data)).catch(() => {}); }
  ...
  const payPayroll = async (id: string) => {
    ...
    const date = prompt('Payment date (YYYY-MM-DD):', new Date().toISOString().split('T')[0]) || new Date().toISOString().split('T')[0];
    try {
      await api.put(`/hr/payroll/${id}/pay`, { payment_account_type: type, payment_date: date });
      refreshAll(); toast.success('Paid');
    } catch (err: any) { toast.error(err.response?.data?.error || 'Error'); }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === 'payslip' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table className="data-table">
            <thead><tr><th>Ref #</th><th>Employee</th><th>Period</th><th>Gross</th><th>Deductions</th><th>Net Pay</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {payroll.map((p) => (
                <tr key={p.id}>
                  <td className="font-mono text-xs">{p.payroll_number}</td>
                  ...
                  <td className="text-xs">{formatDate(p.pay_period_start)} - {formatDate(p.pay_period_end)}</td>
                  <td>{formatCurrency(p.gross_pay)}</td>
                  <td className="text-red-600">{formatCurrency(p.deductions_total)}</td>
                  <td className="font-bold">{formatCurrency(p.net_pay)}</td>
                  <td><span className={statusBadge(p.status)}>{p.status}</span></td>
                  <td>
                    <button onClick={() => viewPayslip(p.id)} className="p-1.5 hover:bg-blue-50 rounded text-blue-600" title="View Payslip"><FileText size={15} /></button>
                  </td>
                </tr>
              ))}
              {payroll.length === 0 && <tr><td colSpan={8} className="text-center text-gray-500 py-6">No payroll records yet</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      {activeTab === 'sss' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table className="data-table">
            <thead><tr><th>Ref #</th><th>Employee</th><th>Period</th><th>Employer Amount</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {sssContributions.map((sc) => (
                <tr key={sc.id}>
                  ...
                  <td className="font-bold text-blue-700">{formatCurrency(sc.employer_amount)}</td>
              ...
              {sssContributions.length === 0 && <tr><td colSpan={6} className="text-center text-gray-500 py-6">No SSS contributions yet</td></tr>}
            </tbody>
          </table>
        </div>
      )}
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <div><label className="block text-sm font-medium mb-1">Location</label>
                  <select value={groceryForm.location_id} onChange={(e) => setGroceryForm({ ...groceryForm, location_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    <option value="">Select</option>
                    <option value="1">Main Store</option><option value="2">Main Warehouse</option><option value="3">Branch 1</option>
                  </select></div>
        ...
        <div className="modal-overlay" onClick={() => setShowPayrollModal(false)}>
          <div className="modal-content max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              ...
              <div className="space-y-3">
                <div><label className="block text-sm font-medium mb-1">Employee</label>
                  <select value={payrollForm.employee_id} onChange={(e) => setPayrollForm({ ...payrollForm, employee_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    <option value="">Select</option>
                    {employees.map((e) => <option key={e.id} value={e.id}>{employeeName(e)}</option>)}
                  </select></div>
                <div className="grid grid-cols-2 gap-2">
                  <div><label className="block text-sm font-medium mb-1">Start Period</label><input type="date" value={payrollForm.pay_period_start} onChange={(e) => setPayrollForm({ ...payrollForm, pay_period_start: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" /></div>
                  <div><label className="block text-sm font-medium mb-1">End Period</label><input type="date" value={payrollForm.pay_period_end} onChange={(e) => setPayrollForm({ ...payrollForm, pay_period_end: e.target.value })} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" /></div>
                </div>
          ...
          <div className="modal-content max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-500 italic">SSS is fully paid by employer. No employee deduction.</p>
              </div>
              <div className="flex justify-end gap-3 mt-6">

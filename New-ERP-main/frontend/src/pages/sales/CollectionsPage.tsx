...
export default function CollectionsPage() {
  ...
  // Recompute cash collected when applied amount or rates change
  const recalcCash = (applied: string, rate: string, lguAmt: string) => {
    ...
    if (r > 0 && !selectedInvoice?.lgu_final_tax) {
      ...
      const cash = Math.max(0, app - parseFloat(ewt_amount || '0') - lg);
  ...
  const submitPayment = async () => {
    ...
    try {
      const res = await api.post('/sales/collections', {
          ...
          <tbody>
            {outstanding.map((inv) => (
              <tr key={inv.invoice_id} className={inv.invoice_id === invoiceParam ? 'bg-blue-50' : ''}>
                ...
                <td><button onClick={() => selectForPayment(inv)} className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200">Collect</button></td>
              </tr>
            ))}
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <table className="data-table">
          ...
          <tbody>
            {collections.map((col) => (
              <tr key={col.id}>
                <td className="font-mono text-xs">{col.receipt_number}</td><td>{col.customer_name || '-'}</td>
            ...
            <div className="p-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 space-y-2 text-sm">
                ...
                {parseFloat(form.ewt_amount) > 0 && (
                  <div className="flex justify-between bg-orange-50 rounded px-2 py-1 -mx-1">
                    <span className="text-gray-700 font-medium">Less: EWT {form.ewt_rate}%</span>
                    <span className="font-bold text-red-600">-{formatCurrency(parseFloat(form.ewt_amount))}</span>
                  </div>
                )}

                {parseFloat(form.lgu_amount) > 0 && (
                  <div className="flex justify-between bg-orange-50 rounded px-2 py-1 -mx-1">
                    <span className="text-gray-700 font-medium">Less: LGU 5% Final VAT</span>
                    <span className="font-bold text-red-600">-{formatCurrency(parseFloat(form.lgu_amount))}</span>
                  </div>
                )}

                <div className="flex justify-between border-t pt-1 border-blue-300">
                  <span className="font-bold text-blue-700">Cash / Bank to Collect</span>
                  <span className="font-bold text-lg text-blue-700">{formatCurrency(parseFloat(form.cash_collected))}</span>
                </div>

                <div className="flex justify-between border-t pt-1 border-blue-300">
                  <span className="font-bold text-gray-700">Total Applied to Invoice</span>
                  <span className="font-bold text-lg text-gray-900">{formatCurrency(parseFloat(form.applied_amount) || 0)}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-gray-500">Remaining After</span>
                  <span className={`font-bold ${(parseFloat(selectedInvoice.balance) - (parseFloat(form.applied_amount) || 0)) <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {formatCurrency(Math.max(0, parseFloat(selectedInvoice.balance) - (parseFloat(form.applied_amount) || 0)))}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div><label className="block text-sm font-medium mb-1">OR / CR Number</label>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button onClick={submitPayment} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Post Collection</button>
              </div>
            </div>
          </div>
        </div>

...
export default function ExpenseList() {
  ...
  const loadExpenses = () => {
    api.get('/expenses').then((res) => setExpenses(res.data.data)).catch((err) => toast.error(err.response?.data?.error || 'Failed to load data'));
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await api.get('/expenses'); setExpenses(res.data.data);
